function [walls, echo_labeling] = getWalls_martin4_2(r, s, dnlos, threshold, gcc_scores, gccPhatSettings, maxIters, allPeaks)
...
while true
    ...
    if isempty(wall) || size(inliers, 1) < 1 %10
        break
    else
...
function [plane, best_inliers] = ransac_plane(r, s, dnlos, iters, threshold_distance, gcc_scores, gccPhatSettings, allPeaks)
    ...
    for iter = 1:iters
        ...
        candidate_planes = wallMinSolver(sample_r, sample_s, sample_d);
